...
`r paste0("Estimated views for Red Notice in 2021: ", red_notice_2021$estimated_views_millions, " million")`
...
climbing_films <- COUNTRY_TOP_10 |>
  ...
  summarise(
    ...
    latest_week  = if (any(weekly_rank == 1)) max(week[weekly_rank == 1]) else as.Date(NA),
    .groups = "drop"
  ) |>
...
# Table output
climbing_films |>
  datatable(options = list(searching = FALSE, info = FALSE, pageLength = 10)) |> formatRound("first_rank", 0)
...
In its debut week, <span style='color:red;'>`r international_debuts$countries_charted`</span> countries featured `r paste0("<span style='color:red;'>", international_debuts$show_title, ": ", international_debuts$season_title, "</span>")` in their Top 10 charts.
...
st_by_season %>%
  datatable(options = list(searching = FALSE, info = FALSE, pageLength = 10)) %>%
  formatRound(c("total_hours", "max_weeks"), 0)
```
...
datatable(options = list(searching = FALSE, info = FALSE, pageLength = 10)) %>%
  formatRound(c("total_weeks", "unique_titles", "avg_weeks_per_title"), 0)
...
international_growth <- GLOBAL_TOP_10 %>%
  ...
  mutate(
    growth_rate = (total_hours / lag(total_hours) - 1) * 100,
...
datatable(options = list(searching = FALSE, info = FALSE, pageLength = 10)) %>%
  formatRound(c("total_hours", "unique_titles", "avg_hours_per_title", "growth_rate"), 1)
```
