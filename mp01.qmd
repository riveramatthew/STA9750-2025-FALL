...
climbing_films <- COUNTRY_TOP_10 |>
  ...
  # Only keep titles that didn't debut at #1 but reached it later
  filter(first_rank > 1 & hit_number_1) |>
  mutate(
    time_to_1_weeks = as.integer(first_week_at_1 - debut_week) / 7
  ) |>
  arrange(desc(first_week_at_1))

# Table output with cleaner columns
climbing_films |>
  select(show_title, debut_week, first_rank, first_week_at_1, time_to_1_weeks) |>
  datatable(
    options = list(searching = FALSE, info = FALSE, pageLength = 10),
    colnames = c("Film Title", "Debut Week", "Debut Rank", "First Week at #1", "Weeks to #1")
  ) |>
  formatRound("first_rank", 0) |>
  formatRound("time_to_1_weeks", 0)
