...
![Netflix Logo](Netflix.png){ width=200px }
...
### Task 6: Netflix India Press Release Analysis

### Supporting R Code for Press Release 2: Commercial Success in India

```{r}

# Analyze India market performance across all content
india_market_stats <- COUNTRY_TOP_10 |>
    ...
    summarise(
        ...
        longest_run = max(max_weeks_top10, na.rm = TRUE),
        avg_weeks = round(mean(max_weeks_top10, na.rm = TRUE), 1)
    )

# Find content exclusive to India market (not in US)
india_exclusive_content <- COUNTRY_TOP_10 |>
    filter(country_name == "India") |>
    anti_join(
        COUNTRY_TOP_10 |> filter(country_name == "United States"),
        by = "show_title"
    ) |>
    summarise(exclusive_titles = n_distinct(show_title))

# International reach of India-popular content
india_international_reach <- COUNTRY_TOP_10 |>
    filter(country_name == "India") |>
    distinct(show_title) |>
    inner_join(
        COUNTRY_TOP_10 |>
            group_by(show_title) |>
            summarise(countries_reached = n_distinct(country_name), .groups = 'drop'),
        by = "show_title"
    ) |>
    summarise(avg_reach = round(mean(countries_reached), 1))
```
