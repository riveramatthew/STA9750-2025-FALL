...
#' FIXED: Fetch CDC PLACES Health Outcome Data (supports year=2023; $where format)
get_cdc_places_nyc <- function(year = 2023, cache_dir = file.path("data", "mp02")) {
    
    # Map release year to endpoint and data year
    endpoint_map <- list(
    ...
    for (measure in measures) {
        ...
        # FIXED: $where with URL encoding for Socrata
        ...
        message(sprintf("  Query URL preview: %s...", substr(query_url, 1, 100)))  # Log for debug
        ...
        if (is.null(response) || httr::status_code(response) != 200) {
            warning(sprintf("API request failed for %s (status: %d)", measure, 
                            ifelse(is.null(response), "NULL", httr::status_code(response))))
            next
        }
        
        # Parse and check for valid data
        raw_content <- httr::content(response, "text", encoding = "UTF-8")
        data <- tryCatch({
            jsonlite::fromJSON(raw_content)
        }, error = function(e) {
            warning(sprintf("JSON parse failed for %s: %s", measure, e$message))
            return(NULL)
        })
        
        # Skip if null, not data.frame, or empty
        if (is.null(data) || !is.data.frame(data) || nrow(data) == 0) {
            warning(sprintf("No valid data returned for %s (rows: %d)", measure, ifelse(is.null(data), 0, nrow(data))))
            next
        }
        
        all_data[[measure]] <- data |>
            select(locationid, data_value) |>  # No 'measure' column (fixed per query)
            rename(GEOID = locationid, !!measure := data_value)
        
        message(sprintf("  Retrieved %d records for %s", nrow(all_data[[measure]]), measure))
    }
    
    # Combine all measures
    if (length(all_data) == 0) {
        stop("No CDC PLACES data retrieved for any measure")
    }
    ...
    # Clean, convert to numeric, and filter to NYC tracts only
    places_data <- places_data |>
        mutate(
            across(all_of(measures), ~as.numeric(as.character(.x))),
            GEOID = as.character(GEOID)
        ) |>
        # Precise NYC filter using GEOID
        filter(str_starts(GEOID, "360")) |>  # NY state tracts
        mutate(county_fips = str_sub(GEOID, 3, 5)) |>
        filter(county_fips %in% c("005", "047", "061", "081", "085")) |>  # NYC counties
        select(-county_fips)
    
    if (nrow(places_data) == 0) {
        stop("No NYC-specific data after filtering")
    }
    
    # Cache for future use
    readr::write_csv(places_data, cache_file)
    message(sprintf("Cached CDC PLACES data to: %s", cache_file))
    message(sprintf("Retrieved data for %d NYC census tracts across %d measures", 
                    nrow(places_data), length(all_data)))
...
# FIXED: Load CDC PLACES health outcome data (year = 2023 release)
CDC_PLACES <- get_cdc_places_nyc(year = 2023)

# FIXED: Merge with TRACTS_SF (use correct column names: DIABETES, BPHIGH, ACCESS2)
TRACTS_SF <- TRACTS_SF |>
    left_join(CDC_PLACES |> sf::st_drop_geometry(), by = "GEOID") |>
    mutate(
        # Ensure numeric and handle NAs (correct column names)
        DIABETES = as.numeric(DIABETES),
        BPHIGH = as.numeric(BPHIGH),
        ACCESS2 = as.numeric(ACCESS2)
    )

# FIXED: Quick validation (use correct column names)
cat("=== CDC PLACES MERGE SUMMARY ===\n")
cat(sprintf("Tracts with diabetes data: %d (%.1f%%)\n", 
            sum(!is.na(TRACTS_SF$DIABETES)), 
            100 * mean(!is.na(TRACTS_SF$DIABETES), na.rm = TRUE)))
cat(sprintf("Tracts with hypertension data: %d (%.1f%%)\n", 
            sum(!is.na(TRACTS_SF$BPHIGH)), 
            100 * mean(!is.na(TRACTS_SF$BPHIGH), na.rm = TRUE)))
cat(sprintf("Tracts with uninsured data: %d (%.1f%%)\n", 
            sum(!is.na(TRACTS_SF$ACCESS2)), 
            100 * mean(!is.na(TRACTS_SF$ACCESS2), na.rm = TRUE)))

# FIXED: Sample merged data (correct column names)
cat("\nSample merged tract data:\n")
TRACTS_SF |> 
    sf::st_drop_geometry() |> 
    select(GEOID, borough, min_dist_miles, DIABETES, BPHIGH, ACCESS2) |> 
    head(5) |> 
    print()

# Data summary (unchanged, already uses DIABETES)
