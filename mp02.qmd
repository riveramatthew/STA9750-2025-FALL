...
::: {.callout .callout-insight}
**Answer:** `r q3_result$name[1]` had the highest average individual income in 2015 at `r scales::dollar(q3_result$avg_individual_income[1], accuracy = 1)`. This reflects the state's concentration of high-wage industries, educated workforce, and relatively high cost of living. The gap between top and bottom states (approximately `r scales::dollar(q3_result$avg_individual_income[1] - q3_result$avg_individual_income[nrow(q3_result)], accuracy = 1)`) underscores persistent regional economic disparities that housing policy alone cannot address.
:::
...
**Answer:** NYC last had the most data scientists (NAICS 5182) in `r max(q4_result$YEAR[grepl("New York", q4_result$NAME)])`, with `r scales::comma(q4_result$EMPLOYMENT[grepl("New York", q4_result$NAME) & q4_result$YEAR == max(q4_result$YEAR[grepl("New York", q4_result$NAME)])])` employed. By `r min(q4_result$YEAR[grepl("San Francisco", q4_result$NAME)])`, San Francisco overtook the lead, reflecting the tech industry's westward migration and concentration in the Bay Area. This shift underscores how housing costs influence talent geography—NYC's persistently high rent burden (Index \~58) makes it increasingly difficult to retain young professionals despite higher nominal wages.
...
::: {.callout .callout-insight}
**Answer:** The finance and insurance sector (NAICS 52) peaked at `r scales::percent(peak_year$finance_fraction, accuracy = 0.1)` of total NYC metropolitan wages in `r peak_year$YEAR`. This dominance highlights Wall Street's role as the economic anchor of the region, generating tax revenue and supporting service industries. However, this concentration also creates vulnerability: when finance booms, housing costs surge and price out essential workers; when it contracts, the entire regional economy suffers. The decline to `r scales::percent(nyc_finance$finance_fraction[nyc_finance$YEAR == 2023], accuracy = 0.1)` by 2023 reflects both post-2008 regulatory changes and the rise of tech/healthcare sectors.
:::
